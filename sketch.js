...
var m, a, reload_Image, reload, gameOver_Image, gameOver;
...
function preload() {
  ...
  reload_Image=loadImage("reload.png");
...
reload=createSprite(400, 380);
reload.addImage("reload", reload_Image);
reload.scale=0.2;
...
function draw() {
  ...
  if(gameState==="play"){
    ...
    if(mousePressedOver(reload)){
      gameState="play";
      score=0;
      man.changeAnimation("man",man_Image);
...
function SpawnUfo() {
  ...
  if(frameCount%60===0){
    ufo=createSprite(800,300,20,50);image.png
